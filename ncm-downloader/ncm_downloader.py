...
def save_config(config):
    with open(f'{current_dir}/config.json', 'w', encoding='utf-8') as f:
...
def download_song(song_id, quality, folder_name, lyric_option):
    ...
    for attempt in range(retries):
        try:
            ...
            suffix = download_res['data'][0]['type']
            music_level = download_res['data'][0]['level']
            print(f"下载品质===》{qualities_name.get(music_level)}")
            ...
            song_filename = os.path.join(folder_name, f"{song_name} - {artist_name}.{suffix}")
            ...
            if lyric_option == '1':
                ...
                lyric_filename = os.path.join(folder_name, f"{song_name} - {artist_name}.lrc")
...
def start_download():
    ...
    if download_type == 'artist':
        ...
        artist_name = artist_albums_res['artist']['name']

        for album in albums:
            album_name = re.sub(r'[<>:"/\\|?*]', '_', album['name'])
            folder_name = f"{current_dir}\\歌手 - {artist_name}\\{album_name}"
        ...
        folder_name = f"{current_dir}\\单曲-{id_value}"
        os.makedirs(folder_name, exist_ok=True)
        download_song(id_value, quality, folder_name, lyric_option)
        ...
        folder_name = f"{current_dir}\\歌单 - {id_value}"
        os.makedirs(folder_name, exist_ok=True)
        download_all(song_ids, folder_name, quality, lyric_option, max_workers)
        ...
        folder_name = f"{current_dir}\\专辑 - {album_name}"
        os.makedirs(folder_name, exist_ok=True)
        download_all(song_ids, folder_name, quality, lyric_option, max_workers)
